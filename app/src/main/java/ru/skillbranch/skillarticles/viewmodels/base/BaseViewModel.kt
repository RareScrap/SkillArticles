...
abstract class BaseViewModel<T: IViewModelState>(initState: T) : ViewModel() {
     ...
     */
    /*
    Мы выносим апдейт стейта на UI поток для того чтобы разделить изменение стейта и его
    асинхронное обновление. И хотя мы могли использовать LiveData#postValue(), но он
    к сожалению не всегда будет отрабатываться корректно при асиннных операциях. Это из-за
    того, что одновременно вызвав post в нескольких бэкграундных потоках мы не можем гарантировать
    порядок их вызовов. Кроме того при вызове post данные с UI тредом будут синхронизированы только
    когда выполнится следующий шаг интерации (что это?). Мы не можем знат наверняка когда это
    произойдет. Из-за этой ситуации тяжело сделать concurency обновления стейта
     */
    @UiThread // TODO: Зачем?
